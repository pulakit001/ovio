 ...
 */
export async function transcribePcmCloud(pcm, keys, { model = "whisper-large-v3-turbo", language = "en", prompt } = {}) {
  ...
  for (const keyObj of activeKeys) {
    ...
    while (attempt < 3) {
      ...
      try {
        ...
        // Optional decoder prompt (kept for API completeness; nothing sets it
        // since voice-profile biasing was removed from Settings).
        if (prompt && String(prompt).trim()) form.append("prompt", String(prompt).trim().slice(0, 224));
