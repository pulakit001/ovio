...
/**
 * Curated shortlist of the 3 best local models for writing study notes,
 * shown in onboarding (and usable anywhere a model pick is needed).
 * - "llama3.2"      : smallest/fastest, great on any Mac
 * - "llama3.1:8b"   : balanced quality/speed, the sweet spot
 * - "qwen2.5:14b"   : deepest, most detailed notes (needs a beefier Mac)
 */
export const OLLAMA_TOP_MODELS = [
  { id: "llama3.2", title: "Llama 3.2 3B", desc: "Fastest · great on any Mac", size: "~2.0 GB", recommended: true },
  { id: "llama3.1:8b", title: "Llama 3.1 8B", desc: "Balanced quality and speed", size: "~4.7 GB" },
  { id: "qwen2.5:14b", title: "Qwen 2.5 14B", desc: "Deepest, most detailed notes", size: "~9.0 GB" },
];
 ...
 */
export async function ollamaChat(url, model, messages, { system, maxTokens = 4096, temperature = 0.3, timeoutMs = 180000, onToken = null } = {}) {
  // When onToken is provided we stream the response: every generated token is
  // forwarded to the caller so the UI progress bar moves in real time instead
  // of sitting frozen while the model writes. The timeout then becomes an
  // INACTIVITY timeout (reset on every token) rather than a hard total limit —
  // a big local model legitimately needs many minutes for a full-length note.
  ...
  const body = {
    ...
    options: {
      ...
      num_ctx: 16384, // long transcripts need more room than the 2048 default (≈12k words per pass)
    },
  };
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (!useStream) {
        ...
        //  tags — strip it, and treat an empty answer as a failure.
      ...
      while (true) {
        ...
        if (done) break;
        arm(); // token arrived — the model is alive, restart the inactivity clock
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          try {
            ...
            continue; // partial/split line — wait for more data
          }
          if (evt.error) throw new Error(`Ollama error: ${evt.error}`);
          const piece = evt.message?.content || "";
          if (piece) {
            text += piece;
            try { onToken(piece, text); } catch {}
          }
          if (evt.done) sawDone = true;
        }
      }
      const clean = stripThink(text);
      if (!clean.trim()) throw new Error("model returned an empty response");
      return clean;
    } catch (err) {
      lastErr = err;
