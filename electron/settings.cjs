...
// The user-facing ambient chords. An empty persisted list must never happen:
// it would leave only the ⌘⇧U safety chord working. Exported so main.cjs can
// heal a wiped list back to these.
const AMBIENT_DEFAULT_CHORDS = ["CommandOrControl+Shift+Space", "Alt+Shift+Space"];

const DEFAULT_SETTINGS = {
  mode: "local",
  sttModel: "whisper-large-v3-turbo",
  localSttModel: "large",
  // One-time flag: when the Parakeet engine finishes installing we promote it
  // to localSttModel and set this, so later user choices are respected.
  parakeetDefaulted: false,
  ...
  ollamaModel: "",
  // ---- Ambient global shortcut (record from anywhere) ----
  ambient: { enabled: true, chords: AMBIENT_DEFAULT_CHORDS },
  // ---- UI animation level ----
  // "full" = everything; "reduced" = transitions only, no loops/pulses;
  // "off" = instant switches, no motion at all.
  animations: "full",
  // UI theme mode: "dark" (default) or "light". Splash + window background
  // read this too, so the very first frame matches the chosen mode.
  theme: "dark",
  // ---- AI behavior ----
  aiBehavior: {
    // Short custom instructions appended to every notes/chat prompt.
    customInstructions: "",
    // The four embedded note sections toggled in Settings → AI Behavior.
    skills: { overview: true, keyPoints: true, detailedSummary: true, followUps: true },
    // "detailed" (default) vs "concise" notes depth.
    depth: "detailed",
  },
  // Bump when the legal text changes so users can be re-notified.
  legalVersion: 1,
};
...
function migrateParsed(parsed) {
  ...
  // Local STT now offers only Parakeet + Whisper Large v3 — migrate legacy
  // picks (small/turbo) to the most accurate Whisper so nothing breaks.
  if (out.localSttModel === "small" || out.localSttModel === "turbo") {
    out.localSttModel = "large";
    migrated = true;
  }
  // Transcription is strictly Local or Cloud: legacy "hybrid" behaved as
  // local-first, so it maps to "local". The AI-notes provider legacy value
  // "ollama" (local-first hybrid routing) maps to the single "localOnly".
  if (out.mode === "hybrid") {
    out.mode = "local";
    migrated = true;
  }
  if (out.aiProvider === "ollama") {
    out.aiProvider = "localOnly";
    migrated = true;
  }
...
function mergeProviderKeys(prevKeys, patchedKeys) {
  ...
  // The patch is AUTHORITATIVE: keys absent from it were removed by the user
  // in the UI and must stay removed. (Re-appending missing previous entries
  // here resurrected deleted keys — the Settings page sent the remaining
  // list and this loop undid the deletion.)
  return merged;
}
...
module.exports = { registerSettingsIpc, loadSettings, getPlainSettings, sanitizeForRenderer, persistSettings, AMBIENT_DEFAULT_CHORDS };
